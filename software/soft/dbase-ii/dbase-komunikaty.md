*DBASE 
                 ...
                 VERSION 1.17 FOR dBASE II v2.41
...
the following key words. 
          ...
          FULL SCREEN     LIMITS       NEW          VIDEO 
          ERRORS          FUNCTIONS    DBASE 
...
Syntax:
  ...
  @ <coords> [SAY <exp> [USING '<picture>']ń [GET <variable> 
       [PICTURE '<picture>']ń
...
>  ACCEPT  -- Prompts user to enter character string  information 
   ...
   Example:. ACCEPT "What is your name?" to NAME
             What is your name?: [user's response stored to SINń
           . ? NAME
             [user's response displayedń  
*EXIT
*APPEND
> APPEND FROM <file> [FOR <exp>]  or  APPEND  FROM  <file>  [SDFń
     [DELIMITEDń [FOR <exp>] -- appends data  FROM  a database or 
...
> COPY  -- Copies the database in use or only  its  structure  TO 
  ...
  COPY TO <file> [SDFń [DELIMITED [WITH <delimiter>]ń [FOR <exp>]
...
> COUNT -- Counts the number of records in database in use  which 
  ...
  Syntax:  COUNT [scopeń [FOR <exp>] [TO <memvar>]
...
> DISPLAY [<scope>]ź<field list>]źFOR<exp list>]źOFFń -- displays 
...
>  GO or GOTO -- positions to a specific record or place  in  the 
  ...
  Syntax: GO or GOTO [RECORD <n>],<n>,[TOPń,[BOTTOMń, or <memvar>
*EXIT
*HELP
...
> WAIT  -- suspends  command file  processing  until  user  input 
       received.
                 ...
                 (e.g. type 'HELP RUNTIME' )

        FULL SCREEN        NEW            ERRORS
        GLOSSARY           FUNCTIONS      INSTALL
        DBASE              LIMITS         RUNTIME
*EXIT
*IF
*ELSE
*ENDIF
...
> INPUT ["<cstring prompt>"] TO <memvar> -- prompts user to enter 
        ...
        Example: INPUT 'Prompt user for input' TO X
                 ...
                 [user input, 12, stored to Xń
                 . ? X
                 12
*EXIT
*INSERT
> INSERT [źBEFOREń [BLANKńń -- inserts a record into the database 
...
> LIST [<scope>]ź<field  list>] [FOR <exp list>]źOFFń -- displays 
...
>  RELEASE -- erases unwanted memory variables and  opens  memory 
       ...
       Syntax: RELEASE [<memvar list>], or [ALLń
               RELEASE ALL LIKE <skeleton>
               RELEASE ALL EXCEPT <skeleton>
                  
       Examples (where N, N1, N2, N10, V7 are variables in use):

               RELEASE ALL LIKE N?  [N10 and V7 will remainń 
               RELEASE ALL LIKE N?? [V7 will remainń
               RELEASE ALL EXCEPT ?1* [N1 and N10 will remainń
*EXIT
*REMARK
...
> REPORT -- used for creating a Report Form File (FRM) for  dis-
      ...
      Syntax: REPORT [FORM <form file>] [<scope>]źTO PRINTń 
                       [FOR <exp>] [PLAINń
*EXIT
*RESET
...
*EXIT
*RESTORE
> RESTORE FROM <file> [ADDITIVEń -- retrieves and  activates  the 
...
*PRIMARY
*SECONDARY
> SELECT [PRIMARY / secondaryń -- allows user to move between the 
...
> SET -- sets dBASE control parameters.
         ...
         Syntax: SET <parameter> [ON or OFFń
...
indicated by uppercase of ON or OFF in each listing.  
      ...
      Example: SET BELL [ON/offń - default value is ON.

> SET ALTERNATE [OFF/onń -- ON sends all  screen  output  (except 
       full-screen)  to  a disk file.  (Must be preceeded by  SET 
       ALTERNATE  TO <file> command).   OFF shuts off  output  to 
       file.

>  SET BELL [ON/offń -- ON rings when invalid data is entered  or 
       data field boundary is passed.  OFF suspends ringing.

> SET CARRY [OFF/onń -- ON repeats data in  current  record  from 
       previous record when using APPEND in Full-screen mode. OFF 
       leaves field blank.

> SET COLON [ON/offń -- ON displays colons to bound  input  vari-
       ables  when  using  Full-screen  display.    OFF  suspends 
       display of colons.

> SET CONFIRM [OFF/onń -- ON disables automatic skipping to  next 
       field  when current field is  filled while in  Full-screen 
       mode.   OFF  requires a wait for <return> before going  to 
       next field.

> SET CONSOLE [ON/offń -- ON sends all  output  to  screen.   OFF 
       suspends all output to screen. System will appear dead.

> SET DEBUG [OFF/onń -- ON sends output created by ECHO and  STEP 
       options to printer. OFF sends this output to the screen.


> SET DELETED  [OFF/onń -- ON  disables  dBASE  from  FINDing  or 
...
> SET ECHO [OFF/onń --  ON  enables  monitoring of  command  file 
       execution by echoing all commands to screen.  OFF sends no 
       report on execution.

> SET EJECT [ON/offń -- ON causes REPORT  command  to  perform  a 
       form-feed  (page  eject) before sending report  output  to 
       printer.  OFF disables the page eject.

> SET ESCAPE [ON/offń -- ON allows user  to  abort  execution  of 
...
> SET EXACT [OFF/onń -- ON requires exact matches in any compari-
...
>  SET  INTENSITY  [ON/offń -- ON enables inverse video  or  dual 
       intensity in Full-screen operations (if allowed  by  hard-
       ware).  OFF disables these features. 

> SET LINKAGE [OFF/onń  -- ON enables movement of record pointers 
...
> SET PRINT [OFF/onń -- ON sends output  to  printer.  OFF  stops 
       printer output.

> SET RAW [OFF/onń --  ON  DISPLAYs  and  LISTs  records  without 
       inserting  spaces  between fields;  OFF inserts  an  extra 
       space between fields.
> SET SCREEN [ON/offń -- ON  enables  Full-screen  operation  for 
       APPEND, EDIT, INSERT, READ, and CREATE commands.  

> SET STEP [OFF/onń -- ON  aids  debugging  of  command  file  by 
       halting  execution after dBASE performs each command.  OFF 
       does not halt execution.

> SET TALK [ON/offń -- ON sends results of command  execution  to 
...
*EXIT
*SORT
>  SORT ON <field> TO <file> [ASCENDING / descendingń -- writes a 
...
> SUM  -- computes and displays the sums of numeric  field(s)  of  
       ...
       Syntax: SUM <field> [,<field2>]ź<scope>]źTO <memvar list>]
                              [FOR <exp>]

       Examples:

 . USE SHOPLIST [fields are ITEM, NUMBER purchased, COST of itemń
...
> UPDATE -- allows batch update of presorted or indexed  database 
   ...
   Syntax: UPDATE FROM <file> ON <key> [ADD <field list>]źREPLACE
            <field list> or <field> WITH <field list>]źRANDOMń
*EXIT
*USE
