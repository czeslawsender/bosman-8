...
and examples of use.  The following notation is used:
        Braces {ż indicate an optional entry.
...
FORMAT:
      ...
      INTEGERłDECIMALłSTRING {(expression)} ARRAY

      identifier ... bound pair list {,identifierż
...
EXAMPLE:
      ...
      INTEGER ARRAY Xź0:5,0:5];

      DECIMAL(10) ARRAY X,Yź3:6,5:10];

      STRING ARRAY  WORDSźY+3:12];

      STRING(20) WORDź0:X+5,5:10,15:20];
...
FORMAT:
      ...
      {label definitionż simple statement



      {label definitionż IF boolean expression THEN balanced
...
FORMAT:
      ...
      [expression : expressionó,expression : expressionż ...]
...
FORMAT:
      ...
      boolean expressionłarithmetic expression
...
FORMAT:
      ...
      {label definitionż FOR assignment statement
...
FORMAT:
      ...
      identifier {(expression {,expressionż ...)}
...
FORMAT:
      ...
      INTEGERłDECIMALłSTRING FUNCTION identifier

      {(identifier {,identifierż ...)}
...
FORMAT:
      ...
      label definition GO TO identifierłinteger

      label definition GOTO identifierłinteger
...
FORMAT:
      ...
      PROCEDURE identifier {(identifier {,identifierż ...)}
...
FORMAT:
      ...
      READ {file identifierż(variable {,variableż ...) {ONENDFILE blockż
...
ELEMENT:
  ...
  FORMAT:
      ...
      blockłassignment statementłfor statementł
      case statementłłgoto statementł
      while statementłread statementłwrite statementł
      procedure callłidentifier
...
FORMAT:
      ...
      INTEGERłDECIMALłSTRING {(identifierłinteger)}
...
-      {label definitionż IF boolean expression THEN statement
...
FORMAT:
      ...
      WRITEłWRITEON {file optionż

      (expressionłtab expressionłstring

      {,expressionłtab expressionłpic definitionłstringż ...)
